...
<div class="container contact">
			...
			function post() {
				grecaptcha.ready(function() {
					grecaptcha.execute('<?php echo $set_recaptchapublickey;?>', {
						...
						xhttp.send("anon=" + anon + "&question=" + answer + "&g-recaptcha-response=" + gtoken);
						xhttp.onload = function() {
  ...
  $npage=$page+1;
  $ppage=$page-1;
  $ucid=$_SESSION["userid"];
  $query =
      "SELECT question, time,uname, id,touid FROM content WHERE touid=NULL OR touid=$ucid OR touid=0 OR uid=$ucid ORDER BY id DESC LIMIT 5 OFFSET $offset";

  ($rs = pg_query($conn, $query)) or die("Cannot execute query: $query\n");

  while ($row = pg_fetch_row($rs)) {
      if ($row[4]!=0) {
          $vc=' <span class="badge bg-danger rounded-circle">DM</span>';
      } else {
          $vc='';
      }
    ...
    <div class="header"><span class="header-conetnt"><i class="fas fa-user"></i> ' .
           htmlentities($row[2]) .$vc.
          '</span> <span class="header-conetnt"><i class="fas fa-clock"></i> ' .
           htmlentities($row[1]) .
          '</span>
		  <span class="header-conetnt">  <button id="share" onclick=\'toggle('.$cpdid.');\'>+Share</button>
</div>
...
<div class="sharefatherpart">
   <ul class="cpt" id="sharelist'.$cpdid.'">
  ...
  ?>
      ...
      echo " disabled";
					...
					<a class="page-link"
						href="/index.php?page=<?php echo $ppage;?>"
						aria-label="Previous">
				...
				<li class="page-item">
					<a class="page-link"
						href="/index.php?page=<?php echo $npage;?>"
						aria-label="Next">
