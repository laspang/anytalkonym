...
class User extends Dbconfig
{
            ...
            if ($this->verifycaptcha($_POST['g-recaptcha-response'])) {
				...
				WHERE email='".$_POST["email"]."'";
                    ...
                    if ($isUserExist) {
                        ...
                        $regex = '/[0-9]*\\.[0-9]+@tmjh\\.tp\\.edu\\.tw/i';//regular expression for email here
				...
				VALUES ('".$_POST["username"]."', '".$_POST["firstname"]."', '".$_POST["lastname"]."', '".$_POST["email"]."', '".password_hash($_POST["passwd"], PASSWORD_DEFAULT)."', '".$authtoken."')";
                            ...
                            if ($userSaved) {
                                ...
                                try {
                                    ...
                                    $link = "<https://anytalkonym.herokuapp.com/verify.php?authtoken=".$authtoken.">Verify Email";
        ...
        if ($_POST['email'] == '') {
            ...
            if ($numRows) {
                ...
                try {
                    ...
                    //Recipients
                    $mail->setFrom('', 'ij SMTP');
                    $mail->addAddress($user['email'], $_POST['firstname'].' '.$_POST['lastname']);     //Add a recipient
                    //Content
                    $mail->isHTML(false);                                  //Set email format to HTML
                    $link="<https://anytalkonym.herokuapp.com/reset_password.php?authtoken=".$authtoken.">Reset Password";
    ...
    {
        $str=("1234567890qwertyuiopasdfghjkl;zxcvbnm,.!@$%^*_+~`QWERTYUIOP|ASDFGHJKL:ZXCVBNM");
        $i=1;
        $word="$";
        while ($i<=50) {
            $word .=substr($str, rand(0, 92), 1);
            $i+=1;
        }
        return $word;
    }
    ...
    public function pingnotify($uid)
    {
        //IT SHOULD BE the submit php and throw to this function
        //nonono i am clear now this will be the uid to notify for general not only the pinging  fcuntion
//err useless function
    }
